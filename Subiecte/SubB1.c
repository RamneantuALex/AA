...
struct Node *deleteUnEven(struct Node *root)
{
    if (root == NULL)
    {
        return NULL;
    ...
    root->left = deleteUnEven(root->left);
    root->right = deleteUnEven(root->right);
...
int isAVL(struct Node *nod)
{
    ...
    if ((left - right) == 0 && isAVL(nod->left) && isAVL(nod->right))
        return 1;
    return 0;
}
int main()
{
    ...
    if (isAVL(root))
    {
        printf("\nArborele   este perfect echilibrat \n");
    }
    else
        printf("\nArborele nu este perfect echilibrat\n");
    root = deleteUnEven(root);
    PostOrder(root);
    return 0;
}